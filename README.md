...
I am using Cachy OS as my base OS. It's a 'relaxed' rolling release based on  Arch Linux for less frequent updates. So far it's working well It makes sense to use a rolling release OS to get the latest packages for LabWC. The marriage between LXQt-LabWC makes sense, Labwc for managing the windows and LXQt for it's utilities, the bar, powermanagement etc. I began using lightweight Wayland window compositors a couple of years back, beginning with Sway and moving onto Hyprland & Wayfire. When Raspberry Pi switched switched from Wayfire to LabWC in their Bookworm release (Oct '24) I decided use it on my laptop as well.  
